...
export default function LocoDashboard() {
    ...
    const phaseOptions = [
        ...
        { label: "Phase 3 (TE Data)", value: PHASE.PHASE_3 }
    ];
    ...
    const fetchScore = async () => {
        try {
            let res = await fetch(`${BACKEND_URL}/api/LocoDash/getScoreList`);
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            if (userRole === "Assessor") {
                res = await fetch(`${BACKEND_URL}/api/LocoDash/getAllLocoDashboard`);
    ...
    const visibleRows = useMemo(() => {
        ...
        if (effectiveRole === "Assessor") {
            rows = rows.filter(
                r =>
                    r.uploadStatus === STATUS.READY_FOR_ASSESSMENT &&
                    (r.phase === PHASE.PHASE_1 || r.phase === PHASE.PHASE_2 || r.phase === PHASE.PHASE_3)
            );
        }
        ...
        if (statusFilter && phaseFilter) {
            rows = rows.filter(
                r => r.uploadStatus === statusFilter && r.phase === phaseFilter
            );
        }

        return rows;
    }, [allRows, effectiveRole, statusFilter, STATUS.READY_FOR_ASSESSMENT, STATUS.INSPECTION_DONE, STATUS.ASSESSED_READY_FOR_UPLOAD, phaseFilter, PHASE.PHASE_1, PHASE.PHASE_2, PHASE.PHASE_3]);
    ...
    const updateConditionScore = async (locoNumber, value) => {
        saveScroll();
        try {
            await fetch(`${BACKEND_URL}/api/LocoDash/updateLocoCondition`, {
                method: "POST",
    ...
    const handleRecalculateClick = async (row) => {
        ...
        try {
            const response = await fetch(`${BACKEND_URL}/api/LocoDash/recalculateLocoValues`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
    ...
    const handleExportToExcel = async () => {
        ...
        const headers = [
            ...
            "Replace Value", "Total Labor Value", "Return to Service Cost", "Benchmarking Value", "Market Value", "Loco Status", "Upload Date",
        ...
        rowsWithId.forEach(row => worksheet.addRow([
            ...
            row.totalValue, row.marketValue, row.assetValue, row.uploadStatus, row.uploadDate,
    ...
    const handleUploadConfirmed = async () => {
        ...
        setUploading(true);
        try {
            const resp = await fetch(`${BACKEND_URL}/api/LocoDash/uploadLocos`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify(payload),
            });
    ...
    const confirmTickWagon = async () => {
        ...
        try {
            ...
            if (
                ...
                url = `${BACKEND_URL}/api/LocoDash/markReadyForAssessmentLoco`;
            }
            else if (
                effectiveRole === "Asset Monitor" &&
                tickTargetRow.uploadStatus === STATUS.INSPECTION_DONE &&
                ...
                url = `${BACKEND_URL}/api/LocoDash/markAssessedReadyForUploadLoco`;
            }
            else if (
                effectiveRole === "Assessor" &&
                tickTargetRow.uploadStatus === STATUS.READY_FOR_ASSESSMENT &&
    ...
    const confirmGeneratePdfs = async () => {
        ...
        let resp = await fetch(`${BACKEND_URL}/api/LocoDash/checkLocoInputs/${parseInt(generatePdfTarget.locoNumber)}`);
    ...
    const renderRowCheckbox = (row) => {
        ...
        const onChange = (e) => {
            ...
            if (
                effectiveRole === "Asset Monitor" &&
                row.uploadStatus === STATUS.INSPECTION_DONE &&
                (row.phase === PHASE.PHASE_1 || row.phase === PHASE.PHASE_2 || row.phase === PHASE.PHASE_3)
            ) {
