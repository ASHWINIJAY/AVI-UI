...
const WagonTankersInspect = () => {
    ...
    useEffect(() => {
        const fetchParts = async () => {
            setLoading(true);
            try {
                const res = await axios.get(`WagonTankersInspect/getParts/${formID}?t=${Date.now()}`);
    ...
    const uploadPhoto = async (file, partType, photoType) => {
        ...
        try {
            ...
            const res = await axios.post("WagonTankersInspect/UploadPhoto", fd, {
    ...
    const deletePhoto = async (photoPath) => {
        if (!photoPath) return;
        try {
            await axios.post("WagonTankersInspect/DeletePhoto", { path: photoPath });
        } catch (ex) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const dtos = rows.map((r) => ({
                ...
                ValveQty: parseInt(r.ValveQty || 0),
...
const twistlocks = (!meta.wagonTwist || meta.wagonTwist === "NULL") ? "N/A" : meta.wagonTwist;
    ...
    const columns = [
        ...
        {
            field: "ValveQty",
            headerName: "Valve Qty",
            width: 140,
            renderCell: (params) => (
                <input
                    type="number"
                    className="form-control form-control-sm"
                    value={params.row.ValveQty ?? ""}
                    onChange={(e) => {
                        const value = e.target.value;
                        setRows(prev =>
                            prev.map(r => r.id === params.row.id ? { ...r, ValveQty: value } : r)
                        );
                    }}
                />
            )
...
export default WagonTankersInspect;
