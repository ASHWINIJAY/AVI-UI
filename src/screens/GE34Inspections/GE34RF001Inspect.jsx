...
const storedUserId = localStorage.getItem("userId") ?? "";
    ...
    useEffect(() => {
        const fetchParts = async () => {
            setLoading(true);
            try {
                const res = await axios.get(`GE34RF001/getParts/${formID}?t=${Date.now()}`);
    ...
    const getPartCost = async (partId, field) => {
        try {
            const res = await axios.get(
                `GE34RF001/getPartCost?partId=${encodeURIComponent(partId)}&field=${encodeURIComponent(field)}`
            );
    ...
    const uploadPhoto = async (file, partId, photoType) => {
        ...
        try {
            ...
            const res = await axios.post("GE34RF001/UploadPhoto", fd, {
    ...
    const deletePhoto = async (photoPath) => {
        if (!photoPath) return;
        try {
            await axios.post("GE34RF001/DeletePhoto", { path: photoPath });
        } catch (ex) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            await axios.post(
                `LocoDash/insertLoco?locoNumber=${encodeURIComponent(parseInt(storedLocoNumber))}&userId=${encodeURIComponent(storedUserId)}`
            );
    ...
    return (
        <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh" }}>
            <Container className="mt-4 mb-4">
                <h3 className="text-center mb-4" style={{ color: "white" }}>Roof Inspect</h3>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginTop: 20 }}>
                    ...
                    <Button variant="success" onClick={handleSubmit} disabled={submitting}>{submitting ? "Submitting..." : "Complete"}</Button>
                </div>
            </Container>
...
export default GE34RF001Inspect;
