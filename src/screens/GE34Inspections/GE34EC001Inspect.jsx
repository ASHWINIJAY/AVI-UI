...
const GE34EC001Inspect = () => {
    ...
    useEffect(() => {
        const fetchParts = async () => {
            setLoading(true);
            try {
                const res = await axios.get(`GE34EC001/getParts/${formID}?t=${Date.now()}`);
    ...
    const getPartCost = async (partId, field) => {
        try {
            const res = await axios.get(
                `GE34EC001/getPartCost?partId=${encodeURIComponent(partId)}&field=${encodeURIComponent(field)}`
            );
    ...
    const uploadPhoto = async (file, partId, photoType) => {
        ...
        try {
            ...
            const res = await axios.post("GE34EC001/UploadPhoto", fd, {
    ...
    const deletePhoto = async (photoPath) => {
        if (!photoPath) return;
        try {
            await axios.post("GE34EC001/DeletePhoto", { path: photoPath });
        } catch (ex) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            await axios.post("GE34EC001/SubmitInspection", dtos,
    ...
    return (
        <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh" }}>
            <Container className="mt-4 mb-4">
                <h3 className="text-center mb-4" style={{ color: "white" }}>Elect Cab Inspect</h3>
...
export default GE34EC001Inspect;
