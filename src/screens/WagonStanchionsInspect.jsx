...
const WagonStanchionsInspect = () => {
    ...
    useEffect(() => {
        const fetchParts = async () => {
            setLoading(true);
            try {
                const res = await axios.get(`WagonStanchionsInspect/getParts/${formID}?t=${Date.now()}`);
    ...
    const uploadPhoto = async (file, partType, photoType) => {
        ...
        try {
            ...
            const res = await axios.post("WagonStanchionsInspect/UploadPhoto", fd, {
    ...
    const deletePhoto = async (photoPath) => {
        if (!photoPath) return;
        try {
            await axios.post("WagonStanchionsInspect/DeletePhoto", { path: photoPath });
        } catch (ex) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const dtos = rows.map((r) => ({
                ...
                StanchionQty: parseInt(r.StanchionQty || 0),
    ...
    const columns = [
        ...
        {
            field: "StanchionQty",
            headerName: "Stanchion Qty",
            width: 140,
            renderCell: (params) => (
                <input
                    type="number"
                    className="form-control form-control-sm"
                    value={params.row.StanchionQty ?? ""}
                    onChange={(e) => {
                        const value = e.target.value;
                        setRows(prev =>
                            prev.map(r => r.id === params.row.id ? { ...r, StanchionQty: value } : r)
                        );
                    }}
                />
            )
    ...
    return (
        <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh" }}>
            <Container className="mt-4 mb-4">
                <h3 className="text-center mb-4" style={{ color: "white" }}>Stanchions Inspect</h3>
...
export default WagonStanchionsInspect;
