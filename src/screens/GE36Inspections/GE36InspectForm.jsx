...
const FORM_ORDER = [
  "BD001", "FL001", "SN001", "CL001", "EC001", "CA001",
   "MG001", "ED001", "CF001", "DE001", "RF001"
];
...
const GE36InspectForm = () => {
  ...
  useEffect(() => {
    const fetchParts = async () => {
      ...
      setError("");
      try {
        const res = await axios.get(`GE36Inspect/getParts/${formID}`);
  ...
  const getPartCost = async (partId, field) => {
    try {
      const res = await axios.get(`GE36Inspect/getPartCost?partId=${encodeURIComponent(partId)}&field=${encodeURIComponent(field)}`);
  ...
  const deletePhoto = async (path) => {
    if (!path) return;
    try {
      await axios.post("GE36Inspect/DeletePhoto", { path });
    } catch (e) {
  ...
  const handleSavePhotoModal = async () => {
    ...
    try {
      const res = await axios.post("GE36Inspect/UploadPhoto", fd, {
...
export default GE36InspectForm;
