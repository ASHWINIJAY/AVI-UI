...
const GE36MG001Inspect = () => {
    ...
    useEffect(() => {
        const fetchParts = async () => {
            setLoading(true);
            try {
                const res = await axios.get(`GE36MG001/getParts/${formID}?t=${Date.now()}`);
    ...
    const getPartCost = async (partId, field) => {
        try {
            const res = await axios.get(
                `GE36MG001/getPartCost?partId=${encodeURIComponent(partId)}&field=${encodeURIComponent(field)}`
            );
    ...
    const uploadPhoto = async (file, partId, photoType) => {
        ...
        try {
            ...
            const res = await axios.post("GE36MG001/UploadPhoto", fd, {
    ...
    const deletePhoto = async (photoPath) => {
        if (!photoPath) return;
        try {
            await axios.post("GE36MG001/DeletePhoto", { path: photoPath });
        } catch (ex) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            await axios.post("GE36MG001/SubmitInspection", dtos,
...
export default GE36MG001Inspect;
