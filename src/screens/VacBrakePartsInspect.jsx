...
const VacBrakePartsInspect = () => {
    ...
    useEffect(() => {
        const fetchParts = async () => {
            setLoading(true);
            try {
                const res = await axios.get(`VacBrakeInspect/getParts/${formID}?t=${Date.now()}`);
    ...
    const getPartCost = async (partId, field) => {
        try {
            const res = await axios.get(
                `VacBrakeInspect/getPartCost?partId=${encodeURIComponent(partId)}&field=${encodeURIComponent(field)}`
            );
    ...
    const uploadPhoto = async (file, partId, photoType) => {
        ...
        try {
            ...
            const res = await axios.post("VacBrakeInspect/UploadPhoto", fd, {
    ...
    const deletePhoto = async (photoPath) => {
        if (!photoPath) return;
        try {
            await axios.post("VacBrakeInspect/DeletePhoto", { path: photoPath });
        } catch (ex) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            //(Luca) Change
            const res = await axios.post("VacBrakeInspect/SubmitInspection", dtos,
            ...
            if (storedWagonType === "Tanker") {
                navigate("/wagontanker");
                return;
            }
            else if (storedWagonType === "Bottom Discharge") {
                navigate("/wagonbottom");
                return;
            }
            else {
    ...
    return (
        <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh" }}>
            <Container className="mt-4 mb-4">
                <h3 className="text-center mb-4" style={{ color: "white" }}>Vac Brake Inspect</h3>
