...
const WagonTwistlocksInspect = () => {
    ...
    useEffect(() => {
        const fetchParts = async () => {
            setLoading(true);
            try {
                const res = await axios.get(`WagonTwistlocksInspect/getParts/${formID}?t=${Date.now()}`);
    ...
    const uploadPhoto = async (file, partType, photoType) => {
        ...
        try {
            ...
            const res = await axios.post("WagonTwistlocksInspect/UploadPhoto", fd, {
    ...
    const deletePhoto = async (photoPath) => {
        if (!photoPath) return;
        try {
            await axios.post("WagonTwistlocksInspect/DeletePhoto", { path: photoPath });
        } catch (ex) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const dtos = rows.map((r) => ({
                ...
                TwistlockQty: parseInt(r.TwistlockQty || 0),
    ...
    const columns = [
        ...
        {
            field: "TwistlockQty",
            headerName: "Twistlock Qty",
            width: 140,
            renderCell: (params) => (
                <input
                    type="number"
                    className="form-control form-control-sm"
                    value={params.row.TwistlockQty ?? ""}
                    onChange={(e) => {
                        const value = e.target.value;
                        setRows(prev =>
                            prev.map(r => r.id === params.row.id ? { ...r, TwistlockQty: value } : r)
                        );
                    }}
                />
            )
                        ...
                        {rows.map((row) => (
                            <div key={row.id} style={{ border: "1px solid #ddd", padding: 12, borderRadius: 8, marginBottom: 12, background: "#fff" }}>
                                ...
                                <div style={{ marginTop: 8 }}>
                                    <input className="form-control form-control-sm" value={row.TwistlockQty} placeholder="Twistlock Qty" />
...
export default WagonTwistlocksInspect;
