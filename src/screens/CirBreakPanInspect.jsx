...
const partDescriptions = [
   ...
   "Ground relay clear light indicator",
   "No power white light indicator ",
   "Low lube green light indicator ",
   "Hot engine red light indicator ",
   "Engine stop switch",
   "Prime start rotary switch",
   "Engine control and run rotary switch (EC)",
   "Control circuit breaker 40-amp 2 pole"
];
...
const CirBreakPanInspect = () => {
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      UserID : userID,
      InspectFormID: "CBI001",
      Rows: formattedRows,
    };
    try {
      await axios.post("http://41.87.206.94/AVIapi/api/CirBreakPanInspect/submit", payload);
...
export default CirBreakPanInspect;
