...
const partDescriptions = [
   "Battery knife switches stationary side",
   "Battery knife switch moveable blade",
   "800 Amp Starting fuse",
   "250 Amp Auxiliary generator charging fuse",
   "Ground relay knife switch",
   "Fuse test panel",
   "Battery knife switch compartment light ",
   "Battery knife switch compartment light switch",
   "EFL test switch"
];

const BatSwitchInspect = () => {
...
const [loading, setLoading] = useState(false);
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      UserID : userID,
      InspectFormID: "BSI001",
      Rows: formattedRows,
    };
    try {
      setLoading(true);
      await api.post("BatSwitchInspect/submit", payload);
      navigate("/leftmiddoorinspect");
    } catch (err) {
         const isOffline =
    !navigator.onLine ||
  ...
  if (isOffline) {
   const offlineData = JSON.parse(localStorage.getItem("offlineBatSwitch") || "[]");
  offlineData.push(payload);
  localStorage.setItem("offlineBatSwitch", JSON.stringify(offlineData));
...
export default BatSwitchInspect;
