...
const partDescriptions = [
   ...
   "DGX Relay",
   "TDS Relay",
   "MRD time delay relays (motoring)",
   "BRD time delay relays (braking)",
   "FOR Relay",
   "RER Relay",
   "ISA Relay"
];
...
const TopRightPanInspect = () => {
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      UserID : userID,
      InspectFormID: "TRI001",
      Rows: formattedRows,
    };
    try {
      await axios.post("http://41.87.206.94/AVIapi/api/TopRightPanInspect/submit", payload);
...
export default TopRightPanInspect;
