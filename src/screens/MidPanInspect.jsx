...
const partDescriptions = [
   "Load test panel module",
   "Voltage regulator Module (VR10)",
   "Nexsys controller box 1 with the screen",
   "Nexsys controller box 2 without screen",
   "Nexsys test control switch panel"
];
...
const [loading, setLoading] = useState(false);
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      UserID : userID,
      InspectFormID: "MPI001",
      Rows: formattedRows,
    };
    try {
      setLoading(true);
      await api.post("MidPanInspect/submit", payload);
      navigate("/botleftpaninspect");
    } catch (err) {
       const isOffline =
    !navigator.onLine ||
  ...
  if (isOffline) {
   const offlineData = JSON.parse(localStorage.getItem("offlineMidPan") || "[]");
  offlineData.push(payload);
  localStorage.setItem("offlineMidPan", JSON.stringify(offlineData));
