...
const partDescriptions = [
   "No.2 End Head light",
   "No.2 End Head light cover",
   "No.2 End Head light box",
   "No.2 End Head light cover glass",
   "No.2 End Head light cover",
   "No.2A Marker light",
   "No.2B Marker Light",
   "No.2 End Sand Box Hand rail A-Side",
   "No.2 End Sand Box Hand rail B-Side"
];

const EndDeckInspect = () => {
...
const [loading, setLoading] = useState(false);
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      UserID : userID,
      InspectFormID: "DEI001",
      Rows: formattedRows,
    };
    try {
      setLoading(true);
      await api.post("EndDeckInspect/submit", payload);
      navigate("/coupgearinspect");
    } catch (err) {
      console.error(err);
       const isOffline =
    !navigator.onLine ||
  ...
  if (isOffline) {
   const offlineData = JSON.parse(localStorage.getItem("offlineEndDeck") || "[]");
  offlineData.push(payload);
  localStorage.setItem("offlineEndDeck", JSON.stringify(offlineData));
