...
const WagonDoorsInspect = () => {
    ...
    useEffect(() => {
        const fetchParts = async () => {
            setLoading(true);
            try {
                const res = await axios.get(`WagonDoorsInspect/getParts/${formID}?t=${Date.now()}`);
    ...
    const getPartCost = async (partType, field) => {
        try {
            const res = await axios.get(
                `WagonDoorsInspect/getPartCost?partType=${encodeURIComponent(partType)}&field=${encodeURIComponent(field)}`
            );
    ...
    const uploadPhoto = async (file, partType, photoType) => {
        ...
        try {
            ...
            const res = await axios.post("WagonDoorsInspect/UploadPhoto", fd, {
    ...
    const deletePhoto = async (photoPath) => {
        if (!photoPath) return;
        try {
            await axios.post("WagonDoorsInspect/DeletePhoto", { path: photoPath });
        } catch (ex) {
        ...
        },
        {
            ...
            renderCell: (params) => (
                <input
                    ...
                    value={params.row.DoorQty ?? ""}
                    onChange={(e) => {
                        const value = e.target.value;
                        setRows(prev =>
                            prev.map(r => r.id === params.row.id ? { ...r, DoorQty: value } : r)
                        );
                    }}
                />
            )
    ...
    return (
        <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh" }}>
            <Container className="mt-4 mb-4">
                <h3 className="text-center mb-4" style={{ color: "white" }}>Wagon Doors Inspect</h3>
...
export default WagonDoorsInspect;
