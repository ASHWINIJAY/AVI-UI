...
const WagonFloorInspect = () => {
    ...
    useEffect(() => {
        const fetchParts = async () => {
            setLoading(true);
            try {
                const res = await axios.get(`WagonFloorInspect/getParts/${formID}?t=${Date.now()}`);
    ...
    const uploadPhoto = async (file, partType, photoType) => {
        ...
        try {
            ...
            const res = await axios.post("WagonFloorInspect/UploadPhoto", fd, {
    ...
    const deletePhoto = async (photoPath) => {
        if (!photoPath) return;
        try {
            await axios.post("WagonFloorInspect/DeletePhoto", { path: photoPath });
        } catch (ex) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const dtos = rows.map((r) => ({
                ...
                SectionQty: parseInt(r.SectionQty || 0),
...
const validateAtLeastOneChecked = () => {
  ...
  return true;
    ...
    const columns = [
        ...
        {
            field: "SectionQty",
            headerName: "Section Qty",
            width: 140,
            renderCell: (params) => (
                <input
                    type="number"
                    className="form-control form-control-sm"
                    value={params.row.SectionQty ?? ""}
                    onChange={(e) => {
                        const value = e.target.value;
                        setRows(prev =>
                            prev.map(r => r.id === params.row.id ? { ...r, SectionQty: value } : r)
                        );
                    }}
                />
            )
