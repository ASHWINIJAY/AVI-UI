...
const [loading, setLoading] = useState(false);
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      UserID : userID,
      InspectFormID: "SNI001",
      Rows: formattedRows,
    };
    try {
      setLoading(true);
      await api.post("ShortNoseInspect/submit", payload);
      navigate("/cablocoinspect");
    } catch (err) {
      console.error(err);
       const isOffline =
    !navigator.onLine ||
  ...
  if (isOffline) {
   const offlineData = JSON.parse(localStorage.getItem("offlineShortNose") || "[]");
  offlineData.push(payload);
  localStorage.setItem("offlineShortNose", JSON.stringify(offlineData));
...
export default ShortNoseInspect;
